...
interface Row {
  load: string; carrier: string; payee: string;
  due_date: string; days_overdue: number; amount: number; currency: string; flagged: boolean;
}
interface Payables {
  rows: Row[]; count: number;
  totals: Record<string, number>;
  aging_buckets: Record<string, Record<string, number>>;
  top_payees: { payee: string; currency: string; amount: number }[];
  as_of: string;
...
export default function PayablesPage() {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      const url = `${API}/api/analytics/payables${force ? "?refresh=true" : ""}`;
    ...
    return (
      <Shell>
        <div className="text-center py-20">
          <p className="text-gray-400 mb-4">{email ? `${email} is not authorized.` : "Sign in to view overdue payables."}</p>
  ...
  return (
    <Shell email={email} onSignOut={signOut}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">Overdue payables</h1>
          <p className="text-sm text-gray-500">Carrier invoices past due that we haven&apos;t paid yet · live from Accounts + ACH/Check</p>
        </div>
      ...
      {loading && !data && <p className="text-gray-500">Loading payables…</p>}

      {data && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <Card label="We owe (USD)" value={fmtFull(totals.USD ?? 0)} color="text-orange-400" sub="past due" />
            <Card label="We owe (CAD)" value={fmtFull(totals.CAD ?? 0)} color="text-orange-400" sub="past due" />
            <Card label="Carrier invoices overdue" value={String(data.count)} color="text-amber-300" />
            <Card label="Worst" value={`${data.rows[0]?.days_overdue ?? 0}d`} color="text-red-400" sub={data.rows[0]?.payee?.slice(0, 22)} />
          </div>

          {data.count === 0 && <p className="text-green-400 mb-8">✓ Nothing overdue — carriers are paid up to date.</p>}
          ...
          {data.count > 0 && (
            <div className="grid lg:grid-cols-2 gap-6 mb-8">
                    ...
                    return (
                      <div key={b}>
                        ...
                        <div className="flex gap-1 h-2.5">
                          <div className="bg-orange-500/80 rounded-sm" style={{ width: `${(usd / maxBucket) * 100}%` }} />
              ...
              <div className="bg-[#141414] border border-[#242424] rounded-xl p-5">
                <div className="text-sm font-semibold mb-4 text-gray-300">Top payees owed</div>
                <div className="space-y-2 max-h-56 overflow-y-auto">
                  {data.top_payees.map((c, i) => (
                    <div key={i} className="flex justify-between text-sm">
                      <span className="text-gray-300 truncate max-w-[220px]">{c.payee}</span>
                      <span className="text-orange-300 whitespace-nowrap">{fmtFull(c.amount)} <span className="text-gray-600 text-xs">{c.currency}</span></span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <>
              ...
              <div className="rounded-xl border border-[#222] overflow-x-auto">
                <table className="w-full text-sm min-w-[560px]">
                  ...
                  <tbody>
                    {rows.map((r) => (
                      <tr key={r.load} className={`border-t border-[#1c1c1c] hover:bg-[#161616] ${r.flagged ? "bg-fuchsia-950/20" : ""}`}>
                        <td className="px-4 py-2 text-gray-400">{r.load}</td>
                        <td className="px-4 py-2 text-gray-200 truncate max-w-[220px]">{r.payee}{r.flagged && <span className="text-fuchsia-400 text-xs"> ⚑ issue</span>}</td>
                        <td className="px-4 py-2 text-gray-500 truncate max-w-[180px]">{r.carrier}</td>
